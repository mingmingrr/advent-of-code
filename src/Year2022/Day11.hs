...
run divider modulus index monkeys = ix index . _1 .~ [] $ foldr throw monkeys items
  where (items, modworry, [check, target1, target2]) = monkeys !! index
        throw item = ix @[Monkey] towards . _1 %~ (item':)
          where item' = modworry item `div` divider `mod` modulus
...
main = do
  ...
  let modulus = product (map (^. _3 . to head) monkeys)
      (rounds, divider) = part2
  print . product . take 2 . sortOn Down . foldl1 (zipWith (+)) . take rounds
    $ unfoldr (Just . runRound (run divider modulus)) monkeys
